...
# Written by Akash Mitra (akashmitra@gmail.com)
# 
...
PHP_SERVER_CONFIG="/etc/php5/fpm/php.ini"
...
aptitude --assume-yes --quiet  install language-pack-en >> /dev/null
export LANGUAGE=en_US.UTF-8
export LANG=en_US.UTF-8
export LC_ALL=en_US.UTF-8
locale-gen en_US.UTF-8                                  >> /dev/null
dpkg-reconfigure locales                                >> /dev/null
aptitude --assume-yes --quiet  update                   >> /dev/null
aptitude --assume-yes --quiet  upgrade                  >> /dev/null
...
aptitude --assume-yes --quiet update                   >> /dev/null
aptitude --assume-yes --quiet install nginx            >> /dev/null
...
aptitude --assume-yes --quiet install php5-fpm php5-gd php-apc php5-mcrypt php5-cli php5-curl 2>&1 1> /dev/null
If_Error_Exit "Can not install PHP fpm"
info "PHP FPM installed successfully"
info "Installing memcached for PHP caching"
aptitude --assume-yes --quiet install memcached php5-memcached                                2>&1 1> /dev/null
...
echo "${CHANGE_STAMP}"  >> /etc/php5/mods-available/apcu.ini
echo "apc.shm_size = ${APC_CACHE_MEM_SIZE}" >> /etc/php5/mods-available/apcu.ini
update_change_log "/etc/php5/mods-available/apcu.ini" "setting APC's memory size to ${APC_CACHE_MEM_SIZE}"
# apc.enabled=1
# apc.shm_segments=1
# apc.shm_size=64M
# apc.ttl=7200
...
info "... defining upstream variable [php5-fpm-sock] under [/etc/nginx/conf.d/]"
echo "upstream php5-fpm-sock {"                 > /etc/nginx/conf.d/php-sock.conf
echo "    server unix:/var/run/php5-fpm.sock;" >> /etc/nginx/conf.d/php-sock.conf
...
echo "                fastcgi_pass php5-fpm-sock;"                                        >> /etc/nginx/sites-available/${SITENAME}
...
aptitude --assume-yes --quiet install mariadb-server php5-mysql                >> /dev/null
